resource "aws_autoscaling_group" "scaling-group-dev" {
  launch_configuration = aws_launch_configuration.template.name
  vpc_zone_identifier  = data.aws_subnets.subnets.ids
  target_group_arns = [aws_lb_target_group.asg-target-dev.arn]
  health_check_type = "ELB"
 
  name                 = "scaling-group-dev"
  min_size = 1
  max_size = 2
...
resource "aws_autoscaling_policy" "Terraform-ASG-Policy" {
  ...
  cooldown               = 60
  autoscaling_group_name = aws_autoscaling_group.scaling-group-dev.name
...
resource "aws_cloudwatch_metric_alarm" "CloudwatchforASG" {
  ...
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.scaling-group-dev.name
  }
