resource "aws_autoscaling_group" "scaling-group-prod" {
  launch_configuration = aws_launch_configuration.template1.name
  vpc_zone_identifier  = data.aws_subnets.subnets.ids
  target_group_arns = [aws_lb_target_group.asg-target-prod.arn]
...
resource "aws_autoscaling_policy" "Terraform-ASG-Policy" {
  ...
  cooldown               = 60
  autoscaling_group_name = aws_autoscaling_group.scaling-group-prod.name
...
resource "aws_cloudwatch_metric_alarm" "CloudwatchforASG" {
  ...
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.scaling-group-prod.name
  }
